...
export function FloatingWidget() {
  ...
  return (
    <>
        ...
        {showScrollTop && (
          <div className="fixed bottom-20 sm:bottom-6 left-4 sm:left-6 z-40">
            <motion.button
              ...
              onClick={scrollToTop}
              className="group relative flex h-14 w-14 items-center justify-center rounded-full bg-slate-900/90 dark:bg-slate-900 border-2 border-slate-700/80 text-teal-400 hover:text-white hover:bg-slate-800 shadow-2xl hover:scale-110 transition-all cursor-pointer backdrop-blur-md"
              title="Go to Top"
              aria-label="Scroll to top of page"
            >
              <ChevronUp size={24} className="group-hover:-translate-y-1 transition-transform" />
              {/* Tooltip */}
              <span className="absolute left-16 whitespace-nowrap rounded-xl bg-slate-950 px-3.5 py-2 text-xs font-bold text-slate-200 opacity-0 group-hover:opacity-100 transition-opacity shadow-xl pointer-events-none border border-slate-800">
                Back to Top
              </span>
            </motion.button>
          </div>
      ...
      <div 
        className="fixed bottom-20 sm:bottom-6 right-4 sm:right-6 z-40 flex flex-col items-end gap-3.5 pointer-events-auto"
        aria-label="Sticky actions widget"
      >
        {/* 2. WATCH REPAIRS LIVE STICKY BUTTON (Positioned right above WhatsApp - High-End TV icon by default, Dot & Text on Hover) */}
        <motion.button
          type="button"
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          onClick={() => setIsLiveModalOpen(true)}
          className="group relative flex h-14 items-center justify-center rounded-full bg-gradient-to-r from-red-600 via-rose-600 to-red-500 text-white shadow-2xl shadow-red-600/50 border-2 border-white transition-all duration-300 cursor-pointer px-4 overflow-hidden"
          title="Watch Repairs Live"
        >
          <LiveTvIcon size={22} className="shrink-0" />
          <div className="flex items-center max-w-0 opacity-0 group-hover:max-w-xs group-hover:opacity-100 transition-all duration-300 ease-in-out group-hover:ml-2.5 whitespace-nowrap overflow-hidden">
            <span className="relative flex h-2.5 w-2.5 shrink-0 mr-2">
        ...
        {expanded && (
          <div className="w-72 sm:w-80 rounded-3xl bg-slate-900/95 backdrop-blur-xl border border-slate-800 p-5 text-white shadow-2xl space-y-4 animate-in fade-in slide-in-from-bottom-3 duration-200">
